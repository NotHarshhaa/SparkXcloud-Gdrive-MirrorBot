...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0, select=False, seed=False):
        ...
        if content_type is None or re_match(r'application/x-bittorrent|application/octet-stream', content_type):
            try:
                ...
                if resp.status_code == 200:
                    ...
                    return sendMessage(f"{tag} 𝐄𝐑𝐑𝐎𝐑: 𝐥𝐢𝐧𝐤 𝐠𝐨𝐭 𝐇𝐓𝐓𝐏 𝐫𝐞𝐬𝐩𝐨𝐧𝐬𝐞: {resp.status_code}", bot, message)
            except Exception as e:
            ...
            msg = "𝐐𝐛 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬 𝐟𝐨𝐫 𝐭𝐨𝐫𝐫𝐞𝐧𝐭𝐬 𝐨𝐧𝐥𝐲. 𝐢𝐟 𝐲𝐨𝐮 𝐚𝐫𝐞 𝐭𝐫𝐲𝐢𝐧𝐠 𝐭𝐨 𝐝𝐨𝐰𝐥𝐨𝐚𝐝 𝐭𝐨𝐫𝐫𝐞𝐧𝐭 𝐭𝐡𝐞𝐧 𝐫𝐞𝐩𝐨𝐫𝐭."
    ...
    if is_gdrive_link(link):
        ...
        Thread(target=qb_dl.add_qb_torrent, args=(link, f'{DOWNLOAD_DIR}{listener.uid}', select)).start()
    else:
        if len(mesg) > 1:
            try:
                ussr = mesg[1]
